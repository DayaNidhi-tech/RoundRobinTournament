...
static void handle_api(SOCKET c,TournamentStore*s,const char*m,const char*p,const char*b){char err[256]={0},name[NAME_LEN]={0},city[NAME_LEN]={0},desc[TEXT_LEN]={0},logo[TEXT_LEN]={0};int id,sub,score1,score2;Tournament*t;
 ...
 if(strcmp(m,"GET")==0&&sscanf(p,"/api/tournaments/%d%c",&id,(char[2]){0})==1){respond_json(c,200,json_tournament(t));return;}
 if(strcmp(m,"PUT")==0&&sscanf(p,"/api/tournaments/%d%c",&id,(char[2]){0})==1){int st=0;json_get_string(b,"name",name,sizeof name);json_get_string(b,"description",desc,sizeof desc);json_get_int(b,"status",&st);respond_json(c,tournament_update(t,name,desc,st,err,sizeof err)?200:400,json_message(err[0]==0,"Tournament updated"));return;}
 if(strcmp(m,"DELETE")==0&&sscanf(p,"/api/tournaments/%d%c",&id,(char[2]){0})==1){respond_json(c,tournament_delete(s,t->id,err,sizeof err)?200:400,json_message(err[0]==0,"Tournament deleted"));return;}
 if(strcmp(m,"GET")==0&&sscanf(p,"/api/tournaments/%d/teams",&id)==1){respond_json(c,200,json_teams(t));return;}
 if(strcmp(m,"POST")==0&&sscanf(p,"/api/tournaments/%d/teams",&id)==1){json_get_string(b,"name",name,sizeof name);json_get_string(b,"city",city,sizeof city);json_get_string(b,"logo",logo,sizeof logo);respond_json(c,team_add(t,name,city,logo,err,sizeof err)?200:400,json_message(err[0]==0,"Team added"));return;}
 if(strcmp(m,"PUT")==0&&sscanf(p,"/api/tournaments/%d/teams/%d",&id,&sub)==2){json_get_string(b,"name",name,sizeof name);json_get_string(b,"city",city,sizeof city);json_get_string(b,"logo",logo,sizeof logo);respond_json(c,team_update(t,sub,name,city,logo,err,sizeof err)?200:400,json_message(err[0]==0,"Team updated"));return;}
 if(strcmp(m,"DELETE")==0&&sscanf(p,"/api/tournaments/%d/teams/%d",&id,&sub)==2){respond_json(c,team_delete(t,sub,err,sizeof err)?200:400,json_message(err[0]==0,"Team deleted"));return;}
 if(strcmp(m,"POST")==0&&sscanf(p,"/api/tournaments/%d/schedule/generate",&id)==1){respond_json(c,schedule_generate(t,err,sizeof err)?200:400,json_message(err[0]==0,"Schedule generated"));return;}
 if(strcmp(m,"GET")==0&&(sscanf(p,"/api/tournaments/%d/schedule",&id)==1||sscanf(p,"/api/tournaments/%d/matches",&id)==1)){respond_json(c,200,json_matches(t));return;}
 if(strcmp(m,"PUT")==0&&sscanf(p,"/api/tournaments/%d/matches/%d/result",&id,&sub)==2){if(!json_get_int(b,"homeScore",&score1)||!json_get_int(b,"awayScore",&score2)){respond_json(c,400,json_message(0,"Both scores are required"));return;}respond_json(c,match_set_result(t,sub,score1,score2,err,sizeof err)?200:400,json_message(err[0]==0,"Result saved"));return;}
 if(strcmp(m,"GET")==0&&sscanf(p,"/api/tournaments/%d/standings",&id)==1){respond_json(c,200,json_standings(t));return;}
 if(strcmp(m,"POST")==0&&sscanf(p,"/api/tournaments/%d/save",&id)==1){respond_json(c,storage_save(s,"data/tournaments.bin",err,sizeof err)?200:400,json_message(err[0]==0,"All tournaments saved"));return;}
 if(strcmp(m,"POST")==0&&sscanf(p,"/api/tournaments/%d/load",&id)==1){respond_json(c,storage_load(s,"data/tournaments.bin",err,sizeof err)?200:400,json_message(err[0]==0,"Saved tournaments loaded"));return;}
